...
from python.torx.module.layer import crxb_Conv2d, crxb_Conv2d_dw
...
class Net(nn.Module):
    def __init__(self, crxb_size, gmin, gmax, gwire, gload, vdd, ir_drop, freq, temp, device, scaler_dw, enable_noise,
        ...
        self.conv2 = crxb_Conv2d_dw(10, 10, kernel_size=5, crxb_size=crxb_size, scaler_dw=scaler_dw,
...
#                                  enable_noise=enable_noise, ir_drop=ir_drop, device=device)
    ...
    def forward(self, x):
        ...
        x = F.relu(F.max_pool2d(self.conv2_drop(self.conv3(self.conv2(x))), 2))
        x = x.view(-1, 320)
        x = F.relu(self.fc1(x))
